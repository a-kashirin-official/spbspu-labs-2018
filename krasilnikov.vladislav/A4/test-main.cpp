...
BOOST_AUTO_TEST_SUITE(Rectangle_Tests)
  ...
  BOOST_AUTO_TEST_CASE(Rotate_FrameRect_Rectangle)
  {
    Rectangle rect(13.0, 7.0, {1.0, 1.0});
    rect.rotate(90);
    BOOST_CHECK_CLOSE_FRACTION(rect.getFrameRect().height, 7.0, DELTA);
    BOOST_CHECK_CLOSE_FRACTION(rect.getFrameRect().width, 13.0, DELTA);
  }
  ...
  {
    Circle circ({1.0, 1.0}, 13.0);
    circ.rotate(90);
    BOOST_CHECK_CLOSE_FRACTION(circ.getRadius(), 13.0, DELTA);
    BOOST_CHECK_CLOSE_FRACTION(circ.getArea(), 13.0 * 13.0 * M_PI, DELTA);
  }
...
BOOST_AUTO_TEST_SUITE(Matrix_Tests)
  ...
  BOOST_AUTO_TEST_CASE(Splitting_Matrix)
  {
    ...
    BOOST_CHECK(layer1[0] == circMPtr);
    BOOST_CHECK(layer1[1] == rectM3Ptr);
    BOOST_CHECK(layer2[0] == rectM1Ptr);
    BOOST_CHECK(layer2[1] == rectM2Ptr);
    BOOST_CHECK(layer3[0] == rectM4Ptr);
  }
BOOST_AUTO_TEST_SUITE_END()
