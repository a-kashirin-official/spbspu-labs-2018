...
point_t orlov::Triangle::getPos() const noexcept
{
  return centre_;
}
...
void orlov::Triangle::scale(const double koef)
{
  if (koef <= 0.0)
  {
  ...
  A_.x = centre_.x + ((A_.x - centre_.x) * koef);
  A_.y = centre_.y + ((A_.y - centre_.y) * koef);
  B_.x = centre_.x + ((B_.x - centre_.x) * koef);
  B_.y = centre_.y + ((B_.y - centre_.y) * koef);
  C_.x = centre_.x + ((C_.x - centre_.x) * koef);
  C_.y = centre_.y + ((C_.y - centre_.y) * koef);
}
