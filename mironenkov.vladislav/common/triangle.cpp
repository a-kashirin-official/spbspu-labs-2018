...
mironenkov::Triangle::Triangle(const point_t & a, const point_t & b, const point_t & c):
...
double mironenkov::Triangle::getArea() const
{
  return abs(0.5 * ((a_.x - c_.x) * (b_.y - c_.y) 
                    - (b_.x - c_.x) * (a_.y - c_.y)));
};

mironenkov::rectangle_t mironenkov::Triangle::getFrameRect() const
{
...
void mironenkov::Triangle::move(const double dx, const double dy)
{
  a_.x += dx;
  a_.y += dy;
  b_.x += dx;
...
void mironenkov::Triangle::move(const point_t & pos)
{
  a_.x -= pos_.x - pos.x;
  a_.y -= pos_.y - pos.y;
  b_.x -= pos_.x - pos.x;
...
void mironenkov::Triangle::scale(const double coeff)
{
  if (coeff <= 0.0)
  {
