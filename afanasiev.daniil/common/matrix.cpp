...
afanasiev::Matrix::Matrix(const std::shared_ptr<Shape> shape):
  elements_(nullptr),
  layersNumber_(0),
  layerSize_(0)
{
...
afanasiev::Matrix::Matrix(const Matrix & matrix):
...
afanasiev::Matrix::Matrix(Matrix && matrix):
  elements_(nullptr),
  layersNumber_(matrix.layersNumber_),
  layerSize_(matrix.layerSize_)
{
...
Matrix & afanasiev::Matrix::operator=(const Matrix & matrix)
{
  if (this != & matrix)
  {
...
Matrix & afanasiev::Matrix::operator=(Matrix && matrix)
{
  if (this != & matrix)
  {
    layersNumber_ = matrix.layersNumber_;
...
std::unique_ptr<std::shared_ptr<Shape>[]> afanasiev::Matrix::operator[](const int index) const
{
...
void afanasiev::Matrix::addShape(const std::shared_ptr<Shape> shape) noexcept
{
  if (layersNumber_ == 0)
  {
...
bool afanasiev::Matrix::checkOverlapping(const int index, std::shared_ptr<Shape> shape) const noexcept
{
