...
BOOST_AUTO_TEST_SUITE(Rectangle_Tests)
  ...
  BOOST_AUTO_TEST_CASE(Change_FrameRect_Rotate_Test_Rect)
  {
    Rectangle rectangle(7.0, 16.0, { 19.64, 8.35 } );
    rectangle.rotate(90);
    BOOST_CHECK_CLOSE_FRACTION(rectangle.getFrameRect().width, 16.0, EPS);
    BOOST_CHECK_CLOSE_FRACTION(rectangle.getFrameRect().height, 7.0, EPS);
  }
...
BOOST_AUTO_TEST_SUITE(Circle_Test)
  ...
  BOOST_AUTO_TEST_CASE(Change_FrameRect_Rotate_Test_Circle)
  {
    Circle circ({ 19.64, 8.35 }, 7.0);
    circ.rotate(90);
    BOOST_CHECK_CLOSE_FRACTION(circ.getFrameRect().width, 14.0, EPS);
    BOOST_CHECK_CLOSE_FRACTION(circ.getFrameRect().height, 14.0, EPS);
  }
BOOST_AUTO_TEST_SUITE_END()
...
BOOST_AUTO_TEST_SUITE(CompositeShapeTests)
  ...
  BOOST_AUTO_TEST_CASE(Change_FrameRect_Rotate_Test_CS)
  {
    Rectangle rectangle(7.0, 16.0, { 19.64, 8.35 });
    std::shared_ptr<Shape> rectanglePtr = std::make_shared<Rectangle>(rectangle);
    CompositeShape compositeShape(rectanglePtr);
    compositeShape.rotate(90);
    BOOST_CHECK_CLOSE_FRACTION(compositeShape.getFrameRect().width, 16.0, EPS);
    BOOST_CHECK_CLOSE_FRACTION(compositeShape.getFrameRect().height, 7.0, EPS);
  }
BOOST_AUTO_TEST_SUITE_END()
...
BOOST_AUTO_TEST_SUITE(MatrixTests)
  ...
  BOOST_AUTO_TEST_CASE(Matrix_Test)
  {
    ...
    Rectangle rectangleM1( 2.0, 2.0, { -2.0, 0.0 } );
    Rectangle rectangleM2( 6.0, 3.0, { 1.0, 1.0 } );
    Rectangle rectangleM3( 2.0, 4.0, { 3.0, 1.0 } );
    Rectangle rectangleM4( 4.0, 4.0, { 3.0, 3.0 } );
    ...
    BOOST_CHECK(layer1[1] == rectanglePtrM3);
    BOOST_CHECK(layer2[0] == rectanglePtrM1);
    BOOST_CHECK(layer2[1] == rectanglePtrM4);
    BOOST_CHECK(layer3[0] == rectanglePtrM2);
  }
BOOST_AUTO_TEST_SUITE_END()
