...
BOOST_AUTO_TEST_CASE(Size)
{
  ...
  kudryavtseva::CompositeShape compos;
  compos.add(&r);
  compos.add(&c);
...
BOOST_AUTO_TEST_CASE(ScaleOfCircle)
{
  ...
  BOOST_CHECK_THROW(compos.scale(-3.0), std::invalid_argument);
}
...
{
  kudryavtseva::Rectangle r({2.0,3.0}, 4.0, 5.0);
  kudryavtseva::point_t pos = r.getFrameRect().pos;
  kudryavtseva::point_t pos1 = r.getPos();
  kudryavtseva::CompositeShape compos(&r);
  compos.move(3.0,2.0);
  BOOST_CHECK_CLOSE(compos.getFrameRect().width, 4.0, EPSILON);
  BOOST_CHECK_CLOSE(compos.getFrameRect().height, 5.0, EPSILON);
  BOOST_CHECK_CLOSE(compos.getFrameRect().pos.x, pos.x, EPSILON);
  BOOST_CHECK_CLOSE(compos.getFrameRect().pos.y, pos.y, EPSILON);
  BOOST_CHECK_CLOSE(compos.getFrameRect().pos.x, pos1.x, EPSILON);
  BOOST_CHECK_CLOSE(compos.getFrameRect().pos.y, pos1.y, EPSILON);
