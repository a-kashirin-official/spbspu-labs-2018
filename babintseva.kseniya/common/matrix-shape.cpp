...
bool MatrixShape::checkIntersection(const std::shared_ptr<Shape> & shape1,
                               const std::shared_ptr<Shape> & shape2)
{
  ...
  rectangle_t shape_1_FrameRect = shape1 -> getFrameRect();
  rectangle_t shape_2_FrameRect = shape2 -> getFrameRect();
